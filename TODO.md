...
- **Iceberg v3 is modelled up to what Spark 3.5 can write.** A deletion vector's Puffin blob is
  ...
  **variant / geometry / geography / timestamp_ns** types, which Spark 3.5 cannot write (no
  ...
  renamed primary key merges across the bucket (`pkr`), and a data-evolution split is stitched
  from each file's projection with the column's source decided by id (`der`), which also
  settled that the latest snapshot reads under the latest schema file. **Nested columns are
  ...
  name mapping's tree is walked for a file with no ids at any level (`migdeep`). A Paimon
