...
- **The suite has almost no oracle.** Every Avro fixture except `example/` is written with
  ...
  every real table. `RealTableFixtureTest` reads the checked-in `example/` tables as the one
  genuine oracle, but they are minimal: one snapshot, one manifest, one unpartitioned data
  file, no deletes, v2 only. **Widen them** — a partitioned table, a merge-on-read table with
  positional and equality delete files, several commits with a compaction, and a v3 table —
  generated by real Spark/Flink and checked in. This is the highest-value test work available.
