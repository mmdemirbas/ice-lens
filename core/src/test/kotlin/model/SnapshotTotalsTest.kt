...
class SnapshotTotalsTest {
    ...
    /** `migrated` and `migdeep` are the exceptions: `add_files` appends a manifest, which carries no byte total, so their `total-files-size` starts at 0 and stays short — see IntegrityFixtureTest. */
    @Test
    fun `every recorded total agrees with the live set at that snapshot`() {
        var compared = 0
        val disagreements = mutableListOf<String>()
        fixtures.forEach { name ->
            if (name == "migrated" || name == "migdeep") return@forEach
