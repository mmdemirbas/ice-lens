---
title: Table-format tooling — evidence base
summary: Five research lanes on Iceberg/Paimon/Delta/Hudi tooling, practitioner pain, spec surface, catalogs and distribution — each lane's absence claims adversarially re-verified.
---

> [!TLDR]
> Dated 2026-08-11. Evidence tiers inline: `run` executed and observed, `src` read in source, `doc` read in the project's own documentation, `weak` secondhand.
>
> - Of 30 absence claims from the first pass, 4 were refuted outright and 13 needed narrowing — section 0 lists every one that changed.
> - Metadata-graph visualization is not an unclaimed position; decoding bounds to typed values is.
> - Which files a scan skipped, and why, is available from no Iceberg surface at any price.
...
```oku-chart
{"type":"scatter","title":"The four gaps — how well the absence is established vs. what it costs to close","x_label":"Cost to close (1 low → 5 high)","y_label":"Absence strength (1 weak → 5 verified across surfaces)","series":[{"label":"Blocked on the bound decoder","color":"accent","data":[{"x":4,"y":5,"label":"3.1 skip-with-reason"},{"x":3,"y":4,"label":"3.2 delete topology"},{"x":3,"y":3,"label":"3.3 snapshot diff"}]},{"label":"Independent","color":"warn","data":[{"x":1,"y":1,"label":"3.4 ancestry"}]}]}
```
...
```oku-compare-grid
{"cards":[{"t":"Clause one — \"what is in my table\" is already served","b":"Supported strongly. 15+ Spark metadata tables, 11 in Trino, DuckDB reaching manifest entries with no engine, PyIceberg as Arrow tables. Weaker for Paimon, whose system tables need Flink or Spark.","verdict":"in","accent":"success"},{"t":"Clause two — \"the graph earns its existence\"","b":"Not established. Two OSS projects already render Iceberg metadata as node-link, no evidence links graph rendering to adoption, and of the three relationships named one is cheap and already done, one is partly impossible, and only pruning is both unserved and derivable.","verdict":"out"},{"t":"What the evidence supports instead","b":"Differentiation comes from derived answers no interface produces, and from an access path most tools lack. The graph is a presentation choice for those answers, not the source of the differentiation.","verdict":"in"}]}
```
...
```oku-chart
{"type":"slope","title":"The two rankings disagree, and the disagreement is the decision (1 = best)","from_label":"Demand + absence + durability","to_label":"Maintainer cost + time to first user","items":[{"label":"A · prune explainer","from":1,"to":2},{"label":"C · Iceberg-only v3 depth","from":2,"to":3},{"label":"B · local-first multi-format","from":3,"to":4},{"label":"D · change the channel","from":4,"to":1}]}
```
