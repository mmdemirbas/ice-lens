...
| Rows | Parquet and Avro via DuckDB (ORC has no DuckDB reader, and the card says so), capped at 50 per file, with `_row_id` and deleted rows marked, and what a read returns when the schema has moved on since the file — by field id, with v3 initial defaults and the name mapping | same, with `_ROW_ID`, the `+I` / `-U` / `+U` / `-D` kind of each key-value row, rows a deletion vector marks struck, and the read projection by the schema the file's own `_SCHEMA_ID` names |
